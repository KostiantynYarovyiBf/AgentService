...
#define mnl_attr_for_each_payload(payload, payload_size)                                                               \
  for((attr) = (payload); mnl_attr_ok((attr), (char*)(payload) + payload_size - (char*)(attr));                        \
      (attr) = mnl_attr_next(attr))
...
static int
mnl_cb_run(const void* buf, size_t numbytes, unsigned int seq, unsigned int portid, mnl_cb_t cb_data, void* data)
{
...
__mnlg_msg_prepare(struct mnlg_socket* nlg, uint8_t cmd, uint16_t flags, uint16_t id, uint8_t version)
{
  struct nlmsghdr* nlh;
...
static int mnlg_socket_recv_run(struct mnlg_socket* nlg, mnl_cb_t data_cb, void* data)
{
  ...
  do
  {
    ...
    if(err <= 0)
      break;
    err =
      mnl_cb_run2(nlg->buf, err, nlg->seq, nlg->portid, data_cb, data, mnlg_cb_array, MNL_ARRAY_SIZE(mnlg_cb_array));
  }
  while(err > 0);
...
again:
  ...
  for(peer = peer ? peer : dev->first_peer; peer; peer = peer->next_peer)
  {
    ...
    if(!mnl_attr_put_check(
         nlh, mnl_ideal_socket_buffer_size(), WGPEER_A_PUBLIC_KEY, sizeof(peer->public_key), peer->public_key))
      goto toobig_peers;
    ...
    if(peer->first_allowedip)
    {
      ...
      for(; allowedip; allowedip = allowedip->next_allowedip)
      {
        ...
        {
          if(!mnl_attr_put_check(
               nlh, mnl_ideal_socket_buffer_size(), WGALLOWEDIP_A_IPADDR, sizeof(allowedip->ip4), &allowedip->ip4))
            goto toobig_allowedips;
        }
        else if(allowedip->family == AF_INET6)
        {
          if(!mnl_attr_put_check(
               nlh, mnl_ideal_socket_buffer_size(), WGALLOWEDIP_A_IPADDR, sizeof(allowedip->ip6), &allowedip->ip6))
            goto toobig_allowedips;
        }
...
static int parse_allowedip(const struct nlattr* attr, void* data)
{
  ...
  {
  case WGALLOWEDIP_A_UNSPEC: break;
...
static int parse_peer(const struct nlattr* attr, void* data)
{
  ...
  case WGPEER_A_TX_BYTES:
    ...
    break;
  case WGPEER_A_ALLOWEDIPS: return mnl_attr_parse_nested(attr, parse_allowedips, peer);
  }
...
static int parse_device(const struct nlattr* attr, void* data)
{
  ...
  case WGDEVICE_A_FWMARK:
    ...
    break;
  case WGDEVICE_A_PEERS: return mnl_attr_parse_nested(attr, parse_peers, device);
  }
...
{
  const uint8_t input[] = {
    (src[0] >> 2) & 63, ((src[0] << 4) | (src[1] >> 4)) & 63, ((src[1] << 2) | (src[2] >> 6)) & 63, src[2] & 63};
  unsigned int i;
